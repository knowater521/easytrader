...
from .log import log
import pywintypes
from fh_tools.win32_utils import find_hwnds_by_matchers


class HTZQClientTrader:
    def __init__(self):
        self.Title = '网上股票交易系统5.0'

    def prepare(self, config_path=None, user=None, password=None, exe_path=r'd:\TradeTools\恒泰金玉满堂\hexin.exe'):
        """
        登陆银河客户端
    ...
    def login(self, user, password, exe_path):
        ...
        for _ in range(60):
            self.close_remainder_window()  # 关闭登录后的“营业部公告”提示框
            if self._has_main_window():
                self._get_handles()
                break
            time.sleep(1)
    ...
    def _has_login_window(self):
        for title in [' - 恒泰证券B1', ' - 北京电信 - 北京电信']:  # 窗口句柄的名称，任务管理器中显示的名称
    ...
    def close_remainder_window(self):
        ...
        while remainder_hwnd is None or remainder_hwnd != 0:
            remainder_hwnd = win32gui.FindWindowEx(None, remainder_hwnd, '#32770', None)
            try:
                ...
                if no_show_msg_item != 0:
                    ...
                    win32gui.PostMessage(no_show_msg_item, win32con.BM_SETCHECK, 1, 0)
    ...
    def _get_handles(self):
        ...
        #  获取主窗体控件 ID
        matcher_list = [('window', lambda x: win32gui.GetClassName(x).find('Afx:400000') == 0),  # 交易窗口框架
                        ('window', lambda x: win32gui.GetClassName(x).find('Afx:400000') == 0),  # 交易窗口
                        ('DlgItem', 0xE900),  # 59648 操作窗口框架
                        ('DlgItem', 0xE900),  # 59648 操作窗口框架
                        ('DlgItem', 0x81),  # 129 左侧操作筐
                        ('DlgItem', 0xC8),  # 200 左部折叠菜单控件
                        ('DlgItem', 0x81),  # 129 左部折叠菜单控件
                        ]
        hwnd_list = find_hwnds_by_matchers(matcher_list)
        if hwnd_list is None:
            raise FileNotFoundError('main windows IDs not found')
        main_frame_hwnd, trade_main_hwnd, operate_frame_hwnd, operate_frame_afx_hwnd, hexin_hwnd, \
        scroll_hwnd, tree_view_hwnd = hwnd_list
        # main_frame_hwnd = win32gui.FindWindow('Afx:400000:0:0:60103eef:0', None)  # 交易窗口框架
        # trade_main_hwnd = win32gui.FindWindowEx(main_frame_hwnd, None, None, self.Title)  # 交易窗口
        # operate_frame_hwnd = win32gui.GetDlgItem(trade_main_hwnd, 0xE900)  # 59648 操作窗口框架
        # operate_frame_afx_hwnd = win32gui.GetDlgItem(operate_frame_hwnd, 0xE900)  # 59648 操作窗口框架
        # hexin_hwnd = win32gui.GetDlgItem(operate_frame_afx_hwnd, 0x81)  # 129 左侧操作筐
        # scroll_hwnd = win32gui.GetDlgItem(hexin_hwnd, 0xC8)  # 200 左部折叠菜单控件
        # tree_view_hwnd = win32gui.GetDlgItem(scroll_hwnd, 0x81)  # 129 左部折叠菜单控件
